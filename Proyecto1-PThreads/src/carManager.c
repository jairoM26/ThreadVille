 ...
 */
int genRandom(){    
    srand((unsigned)counter); //setting the time t as the variable to initialize the root of srand 
    int tmp =  (rand()%8);
    return tmp;
}
...
void* create_cars(void *pBridge){    
    ...
    while(1)
    {
        ...
        if(rand%2==0){
            if(type[rand]==1)
            {
                createCar(newCar, colors[type[rand] -1], models[rand], type[rand],3,1.5,AVARAGE_SPEED+80*rand-2000, "R");                
            }
            else if(type[rand]==2)
            {
                createCar(newCar, colors[type[rand] -1], models[rand], type[rand],3,1.5,AVARAGE_SPEED+80*rand-850, "R");                
            }
            else
            {
            ...
            if(type[rand]==1)
            {
                createCar(newCar, colors[type[rand] -1], models[rand], type[rand],3,1.5,AVARAGE_SPEED+80*rand-2000, "L");                
            }
            else if(type[rand]==2)
            {
                createCar(newCar, colors[type[rand] -1], models[rand], type[rand],3,1.5,AVARAGE_SPEED+80*rand-850, "L"); 
            }
            else
            {
