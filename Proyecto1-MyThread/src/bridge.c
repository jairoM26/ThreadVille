...
void writeSerial(char * data, char * port){
    int fd = -1;
    fd = serialport_init(port, 115200);
    if (fd == -1){
...
void *bridgeChar(void* pBridge){
    ...
    while(1){
        ...
        //ceck if the hardware is available
        if(HARDWARE_AVAILABLE){
            char* data = (char*)calloc(48, sizeof(char));
            sprintf(data, "%s%s%s%s%s:%s%s%s%s%s:%s%s%s%s%s:%d\n",
                        bridge1->inLeftQueue->stringQ, tmp11, bridge1->list.stringInBridge, tmp12, bridge1->inrightQueue->stringQ,
